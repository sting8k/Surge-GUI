...
The project goal is to keep Surge Desktop changes safe, explicit, and validated
as the early-stage app matures into a professional download manager.
...
Harness v0 deliberately does not claim that the early-stage app is complete or
production-ready. New source folders, scripts, tests, CI, migrations, or
infrastructure should still arrive only when a selected story needs them.
...
Product docs describe accepted intent. Implemented behavior should be treated as
complete only when product docs, code, and validation evidence agree.

## Spec Lifecycle

Harness v0 treats human direction as input material, not as a permanent operating
manual. Use accepted input to update product docs, story packets, architecture
decisions, and validation expectations.

Do not keep extending a monolithic spec as the living product plan. Ongoing work
should update the smaller product docs, stories, test matrix, and decision
records.
...
either improve the harness directly or add a proposal to `docs/HARNESS_BACKLOG.md`.

## Future Validation Ladder

The expected validation ladder is:
